		...
		if form.is_valid():
			...
			for result in gpt_results:
				...
				if parsed_result:
					...
					#print the region name to be able to demo it
					print(region_name)
					#get the images for that location
					get_location_images(region_name, 1)
			...
			return render(request, 'travel/result.html', {'results': gpt_results}) 
		...
		if form.is_valid():
			...
			for result in gpt_results:
				...
				if parsed_result:
					...
					#print name to display the URL for demoing
					print(name)
					#print the image url
					get_location_images(name, 1)
			...
			#send back a result page that displays the result generated by GPT to the user based on their inputs
			return render(request, 'travel/result.html', context) 
	#if the user has not submit the form
	else:
...
def recommend_restaurants(request):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			#get a prompt based on the user input, and use that prompt to give it to the GPT model to get a result
			result = generate_text_with_gpt(recommend_restaurant(user_input["region"], options))
