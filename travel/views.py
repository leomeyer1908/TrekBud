...
from .airport import generate_kayak_url
from .book_hotel import generate_hotel_url
import re
import json
		...
		if form.is_valid():
			...
			for result in gpt_results:
				...
				if parsed_result:
					...
					#get the current weather data for the region
					weather_data = get_weather(region_name, datetime.now(), datetime.now(), "imperial")
				...
				#Get Kayak URL
				kayak_url = generate_kayak_url(google_maps_api_key, ip_address, region_name)	

				#Get hotel URL
				hotel_url = generate_hotel_url(region_name)
				
				#Get the results that will be displayed on the web page
				results.append({
					"gpt_result": result,
					"region_name": region_name,
					"image_urls": image_urls,
					"weather_data": weather_data["list"][0] if weather_data else None,
					"kayak_url": kayak_url,
					"hotel_url": hotel_url
				})
...
def recommend_attractions(request):
	...
	if request.method == 'POST':
		...
		form = TouristAttractionForm(initial={"region": initial_region})
...
def make_schedule(request):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			#send back a result page that displays the result generated by GPT to the user based on their inputs
			return render(request, 'travel/schedule_result.html', {'title': title, 'schedule': schedule}) 
	#if the user has not submit the form
	else:
...
def recommend_restaurants(request):
	...
	if request.method == 'POST':
		...
		initial_region = request.GET.get('region', '')
		#load the forms page for generating a schedule
		form = RecommendRestaurantsForm(initial={"region": initial_region})
